/**
 * Correct Polyglot Zobrist Key Calculator
 * Uses the actual Polyglot random numbers from the official source
 */

// Complete Polyglot random number array (781 values)
const POLYGLOT_RANDOM = [
  ...
  0x1FE2CCA76517DB90n, 0xD7504DFA8816EDDBn, 0xB9571FA04DC089C8n, 0x1DDC0325259B27DEn,
  ...
  // Castling rights (offset 768, 4 values)
  0x31D71DCE64B2C310n, 0xF165B587DF898190n, 0xA57E6339DD2CF3A0n, 0x1EF6E6DBB1961EC9n,
  // En passant files (offset 772, 8 values for files a-h)
  // Files a-d (772-775)
  ...
  // Turn to move (offset 780, 1 value)
  0xF8D626AAAF278509n
];

// Piece mapping for Polyglot (piece_to_12 in C code)
// Order: black pawn, white pawn, black knight, white knight, etc.
const PIECE_TO_POLYGLOT = {
  'p': 0,  // black pawn
  'P': 1,  // white pawn
  'n': 2,  // black knight
  'N': 3,  // white knight
...
// Polyglot offsets
const RANDOM_PIECE = 0;      // 768 values (12 pieces * 64 squares)
const RANDOM_CASTLE = 768;   // 4 values
const RANDOM_ENPASSANT = 772; // 8 values
const RANDOM_TURN = 780;     // 1 value

/**
 * Calculate Polyglot Zobrist key from a FEN string
 * @param {string} fen - FEN string
 * @returns {BigInt} 64-bit Polyglot key
 */
function calculatePolyglotKey(fen) {
    ...
    for (let char of rows[row]) {
      if (char >= '1' && char <= '8') {
        ...
        if (pieceType !== undefined) {
          const index = RANDOM_PIECE + pieceType * 64 + square;
          key ^= POLYGLOT_RANDOM[index];
        }
        col++;
      }
    }
  ...
  if (enPassant !== '-') {
    ...
    if (file >= 0 && file < 8) {
      // The polyglot source shows it only uses the file, not the full square
      // And the array starts at 772, but some implementations skip index 0
      // Let's check the actual implementation...
      // From the code: RandomEnPassant + square_file(square)
      // And the en passant values start at offset 772
